...
// ── Supabase ────────────────────────────────────────────────────────────────
const SUPABASE_URL = "https://rgopwfgbdwdsvmowogit.supabase.co";
const SUPABASE_KEY = "sb_publishable_2CpkE8EWq26RGc9l-96bww_SmlHYbWg";
...
export default function App() {
  ...
  const { useState: _u, ...React } = { useState };
  useEffect(() => {
    const loadFromSupabase = async () => {
      try {
        // Load patients
        const pts = await sbFetch("patients?select=*&order=therapist,name");
        if (pts && pts.length > 0) {
          setPatients(pts);
          saveToStorage("cv_patients", pts);
        } else {
          // First time: upload initial patients
          const chunks = [];
          for (let i = 0; i < INIT_PATIENTS.length; i += 50) chunks.push(INIT_PATIENTS.slice(i, i+50));
          for (const chunk of chunks) {
            await sbFetch("patients", { method: "POST", body: JSON.stringify(chunk) });
          }
        }
        // Load schedules
        const schs = await sbFetch("schedules?select=*");
        ...
        setDbReady(true); // Use local data as fallback
      }
    };
    loadFromSupabase();
  }, []);
  ...
  const updateCell = (t,d,h,v) => setMonthlySchedules(prev => {
    ...
    // Sync to Supabase
    sbFetch(`schedules?month_key=eq.${monthKey}`, {
      method: "POST",
      headers: { "Prefer": "resolution=merge-duplicates" },
      body: JSON.stringify({ month_key: monthKey, data: updated[monthKey], updated_at: new Date().toISOString() })
    });
    return updated;
  });
  ...
  const addPatient = async () => {
    ...
    const newP = { ...newPatient, id: Date.now() };
    setPatients(prev => {
      const updated = [...prev, newP];
      saveToStorage("cv_patients", updated);
      return updated;
    });
    await sbFetch("patients", { method: "POST", body: JSON.stringify([newP]) });
  ...
  const saveEditPatient = async () => {
    ...
    await sbFetch(`patients?id=eq.${editingPatient.id}`, { method: "PATCH", body: JSON.stringify(editingPatient) });
    ...
    <div style={{ fontFamily:"'Inter',system-ui,sans-serif", background:C.cream, minHeight:"100vh", color:C.dark }}>
      ...
      <div style={{ background:C.dark, padding:"14px 24px", display:"flex", alignItems:"center", justifyContent:"space-between", flexWrap:"wrap", gap:12 }}>
        ...
        <div style={{ display:"flex", gap:6, alignItems:"center" }}>
          ...
          <span style={{ fontSize:10, color: dbReady ? "#A8C5B0" : "#E8A87C", marginRight:4 }}>
            {dbReady ? "● sincronizado" : "● conectando..."}
          </span>
