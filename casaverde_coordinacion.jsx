...
// ── Persistencia local ──────────────────────────────────────────────────────
const STORAGE_KEYS = {
  patients: "cv_patients",
  schedules: "cv_schedules",
};
...
export default function App() {
  ...
  const [monthlySchedules, setMonthlySchedules] = useState(() => loadFromStorage(STORAGE_KEYS.schedules) || { "2026-5": initSchedule() });
  const [activeTherapist, setActiveTherapist] = useState("Laura");
  const [patients, setPatients] = useState(() => loadFromStorage(STORAGE_KEYS.patients) || INIT_PATIENTS);
